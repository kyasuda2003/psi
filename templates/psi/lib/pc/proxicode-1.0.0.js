(function () {
    ...
    function api() {
        ...
        this.loadEventList = function () {
            var _ref = this;
            return $.ajax({url: "{0}{1}/schedulers/".format(app.settings.apihost, app.settings.apipath),
                headers: {Authorization: "Token " + _userInfo.token}})
                    .done(function (data, textStatus, jqxhr) {
                        if (jqxhr.status == "200")
        ...
        this.submitAScheduler=function(scheduler){
            return $.ajax({type:"POST",
                           url:"{0}{1}/schedulers/".format(app.settings.apihost, app.settings.apipath),
                           data: scheduler,
                           headers: {Authorization: "Token " + _userInfo.token}})
                    .fail(function(xhr, err, obj){
                        console.log('Submitting the scheduler failed.');
                    });
        };
